...
impl BusDeviceFunction {
    ...
    pub fn fmt_common(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "/pci/bus/{:#04X}/device/{:#04X}/fucntion/{:#03X}",
            self.bus(),
            self.device(),
            self.function(),
        )
...
impl Pci {
    ...
    pub fn try_bar0_mem64(&self, bdf: BusDeviceFunction) -> Result<BarMem64> {
        ...
        if bar0 & 0b0111 == 0b0100 {
            ...
            let addr_val = (bar0_32 & !0b1111) as u64;
            let addr = addr_val as *mut u8;
            self.write_register_u32(bdf, 0x10, !0u32)?;
            let readed: u32 = self.read_register_u32(bdf, 0x10)? & !0b1111u32;
            let size = readed.wrapping_neg() as u64;
