...
#[repr(packed)]
pub struct AcipHpetDescriptor {
    _header: SystemDescriptionTableHeader,
    _reserved0: u32,
    address: GenericAddress,
    _reserved1: u32,
}
impl AcpiTable for AcipHpetDescriptor {
    const SIGNATURE: &'static [u8; 4] = b"HPET";
    type Table = Self;
}
impl AcipHpetDescriptor {
...
const _: () = assert!(size_of::<AcipHpetDescriptor>() == 56);
...
impl AcpiRsdpStruct {
    ...
    pub fn hpet(&self) -> Option<&AcipHpetDescriptor> {
        let xsdt = self.xsdt();
        xsdt.find_table(b"HPET").map(AcipHpetDescriptor::new)
    }
