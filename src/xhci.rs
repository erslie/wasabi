...
impl PciXhciDriver {
    ...
    async fn run(xhc: Controller) -> Result<()> {
        ...
        if let Some(port) = connected_port {
            ...
            if let Ok(e) = usb::request_string_descriptor_zero(&xhc, slot, &mut ctrl_ep_ring).await
            {
                ...
                let descriptors =
                    usb::request_config_descriptor_and_reset(&xhc, slot, &mut ctrl_ep_ring).await?;
    ...
    async fn address_device(xhc: &Rc<Controller>, port: usize, slot: u8) -> Result<CommandRing> {
        ...
        input_context.as_mut().set_eq_ctx(
            1,
...
impl DeviceContextBaseAddressArray {
    fn new(scratchpad_bufs: ScratchpadBuffers) -> Self {
        ...
        Self {
            ...
            _scratchpad_buffers: scratchpad_bufs,
        }
    }
...
impl InputContext {
    fn set_eq_ctx(self: &mut Pin<&mut Self>, dci: usize, ep_ctx: EndpointContext) -> Result<()> {
...
impl SetupStageTrb {
    ...
    pub const REQ_TYEP_TO_DEVICE: u8 = 0;
